  ...
  return (
    ...
    >
      <List.Dropdown.Section title="Status">
        {statuses.map((status) => (
          <List.Dropdown.Item key={`status-${status}`} title={`${status.substring(0, 1).toLocaleUpperCase()}${status.substring(1)}`} value={status} />
        ))}
...
function DownloadEntry({task, onToggleDetail, showingDetail}: {task: TasksEntity, onToggleDetail: () => void, showingDetail: boolean}) {
  const formatMetadata = useMemo(() => {
    ...
    metadataArray.push({
        title: "Size Downloaded",
        value: formatValue(transfer.size_downloaded),
    });
            ...
            ? {
                detail: (
                  <List.Item.Detail
                    ...
                    markdown={`\n\n${JSON.stringify(task.additional, null, 2)}`}
                  />
                ),
              }
            : { accessories: [{ text: [bytesToSize(task.size)].join(' ') }] };
          
  return <List.Item title={task.title} subtitle={task.status} {...props}
  actions={
      <ActionPanel>
